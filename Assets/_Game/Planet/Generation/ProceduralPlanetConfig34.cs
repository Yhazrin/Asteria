...
{
    /// <summary>
    /// Configuration with splitting parameters.
    /// </summary>
    [CreateAssetMenu(menuName = "Asteria/Procedural Planet Config 34")]
    public sealed class ProceduralPlanetConfig34 : ScriptableObject
    {
        [Header("Planet")]
        ...
        public Mesh GenerateMesh()
        {
            ...
            for (int lat = 0; lat <= resolution; lat++)
            {
                ...
                for (int lon = 0; lon <= resolution; lon++)
                {
                    ...
                    if (enableSplitting && dir.y < splitHeight)
                    {
                        dir.y = splitHeight;
                        dir = dir.normalized;
                    }
            ...
            var mesh = new Mesh { name = $"{planetName}_Split" };
