...
{
    /// <summary>
    /// Configuration with UV parameters.
    /// </summary>
    [CreateAssetMenu(menuName = "Asteria/Procedural Planet Config 8")]
    public sealed class ProceduralPlanetConfig8 : ScriptableObject
    {
        [Header("Planet")]
        ...
        /// <summary>
        /// Generate mesh with custom UVs from config.
        /// </summary>
        public Mesh GenerateMeshWithUVs()
        {
            ...
            for (int lat = 0; lat <= resolution; lat++)
            {
                ...
                for (int lon = 0; lon <= resolution; lon++)
                {
                    ...
                    float u = (theta / (Mathf.PI * 2f)) * uvScale + uvOffset;
                    float v = (phi / Mathf.PI) * uvScale + uvOffset;
            ...
            var mesh = new Mesh { name = $"{planetName}_UVs" };
