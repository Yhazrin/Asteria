...
{
    /// <summary>
    /// Configuration with mesh variant parameters.
    /// </summary>
    [CreateAssetMenu(menuName = "Asteria/Procedural Planet Config 11")]
    public sealed class ProceduralPlanetConfig11 : ScriptableObject
    {
        [Header("Planet")]
        ...
        public int[] variantResolutions = { 128, 64, 32, 16 };
        public float meshScale = 1f;

        /// <summary>
        /// Generate mesh variants from config.
        /// </summary>
        public Mesh[] GenerateVariants()
        {
            var meshes = new Mesh[variantResolutions.Length];

            for (int i = 0; i < variantResolutions.Length; i++)
            {
                meshes[i] = GenerateMesh(variantResolutions[i]);
            }
        ...
        Mesh GenerateMesh(int resolution)
        {
            ...
            var mesh = new Mesh { name = $"{planetName}_{resolution}" };
