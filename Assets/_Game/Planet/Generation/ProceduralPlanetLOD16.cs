...
{
    /// <summary>
    /// LOD system with mesh caching.
    /// </summary>
    public sealed class ProceduralPlanetLOD16 : MonoBehaviour
    {
        [Header("References")]
        ...
        /// <summary>
        /// Get or generate cached mesh for LOD level.
        /// </summary>
        public Mesh GetCachedMesh(int lodLevel, int resolution)
        {
            if (_meshCache.TryGetValue(lodLevel, out var cached))
            {
                return cached;
            }

            var mesh = GenerateMesh(resolution);
            _meshCache[lodLevel] = mesh;
            return mesh;
        }
        ...
        Mesh GenerateMesh(int resolution)
        {
            ...
            for (int lat = 0; lat <= resolution; lat++)
            {
                ...
                for (int lon = 0; lon <= resolution; lon++)
                {
                    ...
                    vertices[idx] = dir * (planet != null ? planet.Radius : 300f);
                    normals[idx] = dir;
            ...
            var mesh = new Mesh { name = $"Cached_LOD_{resolution}" };
