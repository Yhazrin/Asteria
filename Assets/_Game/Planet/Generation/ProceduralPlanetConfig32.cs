...
{
    /// <summary>
    /// Configuration with clipping parameters.
    /// </summary>
    [CreateAssetMenu(menuName = "Asteria/Procedural Planet Config 32")]
    public sealed class ProceduralPlanetConfig32 : ScriptableObject
    {
        [Header("Planet")]
        ...
        public Mesh GenerateMesh()
        {
            ...
            for (int lat = 0; lat <= resolution; lat++)
            {
                ...
                for (int lon = 0; lon <= resolution; lon++)
                {
                    ...
                    // Apply clipping
                    if (enableClipping && dir.y < clipHeight)
                    {
                        dir.y = clipHeight;
                        dir = dir.normalized;
                    }
            ...
            var mesh = new Mesh { name = $"{planetName}_Clipped" };
