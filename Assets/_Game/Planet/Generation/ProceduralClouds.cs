...
{
    /// <summary>
    /// Generates procedural cloud formations around the planet.
    /// Clouds move with wind and cast shadows.
    /// </summary>
    public sealed class ProceduralClouds : MonoBehaviour
    {
        [Header("Settings")]
        ...
        void CreateCloudMaterial()
        {
            ...
            _cloudMaterial.renderQueue = 3000; // Transparent
        }
        ...
        void MoveClouds()
        {
            ...
            foreach (var cloud in _clouds)
            {
                ...
                // Add wobble
                float wobble = Mathf.Sin(time * 0.5f + cloud.wobble) * 2f;
                Vector3 up = planet.GetSurfaceUp(cloud.root.transform.position);
                cloud.root.transform.position += up * wobble * Time.deltaTime * 0.1f;
            }
        }
