...
{
    /// <summary>
    /// Configuration with tangent parameters.
    /// </summary>
    [CreateAssetMenu(menuName = "Asteria/Procedural Planet Config 9")]
    public sealed class ProceduralPlanetConfig9 : ScriptableObject
    {
        [Header("Planet")]
        ...
        /// <summary>
        /// Generate mesh with tangents from config.
        /// </summary>
        public Mesh GenerateMesh()
        {
            ...
            for (int lat = 0; lat <= resolution; lat++)
            {
                ...
                for (int lon = 0; lon <= resolution; lon++)
                {
                    ...
                    if (generateTangents)
                    {
                        Vector3 tangent = new Vector3(-Mathf.Sin(theta), 0, Mathf.Cos(theta)).normalized;
                        tangents[idx] = new Vector4(tangent.x, tangent.y, tangent.z, 1f);
                    }
                }
            }
            ...
            var mesh = new Mesh { name = $"{planetName}_Tangents" };
            mesh.vertices = vertices;
            mesh.normals = normals;
            if (generateTangents) mesh.tangents = tangents;
            mesh.uv = uvs;
